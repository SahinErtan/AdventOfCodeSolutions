...
int main(){
    ...
    while (std::getline(File, rearrangement_procedure)) {
        ...
        if (continue_permission) {
            ...
            while (count > 0) {
                index = crate_stack_arrangement_map[move_from].size() - count;
                crate_stack_arrangement_map[move_to].push_back(crate_stack_arrangement_map[move_from].at(index));
                crate_stack_arrangement_map[move_from].erase(crate_stack_arrangement_map[move_from].begin() + index);
                count--;
            }
        }
