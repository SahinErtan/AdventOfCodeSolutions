    ...
    for (std::string file : file_list) {
        if (file[0] == '/') {
            if(is_exist_on_file_size(file)) {
                total_size = total_size + file_size_map[directory + file];
            }
            else {
                total_size = total_size + calculate_recursive_file_size(directory + file);
            }
        }
        else {
            total_size = total_size + file_size_map[directory + "/" + file];
...
int main(){
    ...
    for (std::map<std::string, std::vector<std::string>>::iterator it = path_directory_map.begin(); it != path_directory_map.end(); it++) {
        file_size_map[it->first] = calculate_recursive_file_size(it->first);
    }
